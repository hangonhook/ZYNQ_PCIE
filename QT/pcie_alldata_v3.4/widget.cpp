    ...
    validDataSize(0)             // 初始化有效数据大小
{
    ui->setupUi(this);
    // 窗口设置
    this->setWindowTitle("PCIe图像助手_所有值(SAMP=256)_v3.4");
...
int Widget::openpcie()
{
    ...
    if (res == "256X256") {
        image_h = 256; image_v = 256; samp_point = 256;
    } else if (res == "512X512") {
        image_h = 512; image_v = 512; samp_point = 256;
    } else { // 255X256
        image_h = 255; image_v = 256; samp_point = 256;
    }
