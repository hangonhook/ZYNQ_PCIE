    ...
    validDataSize(0)             // 初始化有效数据大小
{
    ui->setupUi(this);
    // 窗口设置
    this->setWindowTitle("PCIe图像助手_所有值(SAMP=128)_v3.5");
...
int Widget::openpcie()
{
    ...
    if (res == "256X256") {
        image_h = 256; image_v = 256; samp_point = 128;
    } else if (res == "512X512") {
        image_h = 512; image_v = 512; samp_point = 128;
    } else { // 255X256
        image_h = 255; image_v = 256; samp_point = 128;
    }
    ...
    if(ui->comboBox->currentIndex()==0){        // 确保在读取图像模式
        ...
        recvBuffer = (unsigned int *)__mingw_aligned_malloc(bufferSize + 8192 , 4096); // 4KB对齐 多分配8KB缓冲空间
...
void Widget::processSingleFrame() {
    ...
    // 3. 接收数据
    if(sent > 0) {
        int recvd = fpga_recv(fpga, chnl, recvBuffer, numWords + 512, 15000);
